...
{
  m_Device->FreeSrvUavDescriptor(m_Srv);
}
...
void AccelerationStructure::Create(const AccelerationStructureDesc& desc)
{
...

  {
    ...
    m_ScratchBuffer = m_Device->CreateBuffer(scratchDesc);
  }

  {
    ...
    m_Buffer = m_Device->CreateBuffer(resultDesc);
  }

  {
    ...
    m_Srv = m_Device->AllocCbvSrvUavDescriptor();
