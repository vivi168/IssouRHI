...
void RenderPipelineImpl::Create(const RenderPipelineDesc& desc)
{
  ...
  switch (m_Type) {
    case Type::Render: {
      ...
      CHECK_HR(ToBackend(m_Device)->GetNativeDevice()->CreateGraphicsPipelineState(&psoDesc, IID_PPV_ARGS(&pipelineStateObject)));
      break;
    }
    case Type::Mesh: {
      ...
      CHECK_HR(ToBackend(m_Device)->GetNativeDevice()->CreatePipelineState(&streamDesc, IID_PPV_ARGS(&pipelineStateObject)));
      break;
    }
    default:
      std::unreachable();
...
RayTracingPipelineImpl::RayTracingPipelineImpl(Device* device) : RayTracingPipeline(device) {}

RayTracingPipelineImpl::~RayTracingPipelineImpl() = default;
...
void RayTracingPipelineImpl::Create(const RayTracingPipelineDesc& desc)
{
  ...
  globalRootSignature->SetRootSignature(ToBackend(m_Device)->RootSignature());
  ...
  CHECK_HR(ToBackend(m_Device)->GetNativeDevice()->CreateStateObject(raytracingPipeline, IID_PPV_ARGS(&m_StateObject)));
...
void* RayTracingPipelineImpl::ShaderIdentifier(std::string entryPoint) const
{
  // TODO: cache result?
