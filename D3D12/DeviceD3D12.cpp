...
void DeviceImpl::Create(const GPUSelection& gpuSelection)
{
  ...
  // Misc
  {
    ToBackend(m_Queue.get())->GetNativeQueue()->GetTimestampFrequency(&m_TimestampFrequencyHz);
  }
...
{
  auto qs = std::make_shared<QuerySetImpl>(this, desc);
  qs->Create();
...
{
  auto buf = std::make_shared<BufferImpl>(this, desc);
  buf->Create();
...
{
  auto as = std::make_shared<AccelerationStructureImpl>(this);
  as->Create(desc);
...
{
  auto meshPipeline = std::make_shared<RenderPipelineImpl>(this, RenderPipeline::Type::Mesh);
...
{
  auto rayTracingPipeline = std::make_shared<RayTracingPipelineImpl>(this);
...
{
  auto shaderTable = std::make_shared<ShaderTableImpl>(this);
  shaderTable->Create(desc);

  return shaderTable;
}

DescriptorAllocation DeviceImpl::AllocCbvSrvUavDescriptor()
{
  return m_CbvSrvUavDescriptorHeap.Alloc();
}

DescriptorAllocation DeviceImpl::AllocRtvDescriptor()
{
  return m_RtvDescriptorHeap.Alloc();
}

DescriptorAllocation DeviceImpl::AllocDsvDescriptor()
{
...
void DeviceImpl::FreeRtvDescriptor(DescriptorAllocation alloc)
{
