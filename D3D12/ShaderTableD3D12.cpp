...
namespace D3D12
{
ShaderTableImpl::ShaderTableImpl(Device* device) : ShaderTable(device) {}

ShaderTableImpl::~ShaderTableImpl() = default;
...
void ShaderTableImpl::Create(const ShaderTableDesc& desc)
{
  ...
  auto writeShaderIdentifier = [&](void* dst, const std::string& entryPoint) {
    void* identifier = ToBackend(desc.pipeline)->ShaderIdentifier(entryPoint);
...
D3D12_GPU_VIRTUAL_ADDRESS_RANGE ShaderTableImpl::RayGenShaderRecord() const
{
...
D3D12_GPU_VIRTUAL_ADDRESS_RANGE_AND_STRIDE ShaderTableImpl::MissShaderTable() const
{
...
D3D12_GPU_VIRTUAL_ADDRESS_RANGE_AND_STRIDE ShaderTableImpl::HitGroupTable() const
{
...
D3D12_GPU_VIRTUAL_ADDRESS_RANGE_AND_STRIDE ShaderTableImpl::CallableShaderTable() const
{
